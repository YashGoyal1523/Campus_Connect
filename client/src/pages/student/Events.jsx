      ...
      {filtered.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map((e) => {
            ...
            return (
              <div key={e._id} className="rounded-xl bg-white/5 border border-white/10 hover:border-purple-500/40 transition overflow-hidden">
                ...
                <div className="p-3">
                  ...
                  <p className="text-white/50 text-xs leading-relaxed mb-3 line-clamp-2">{e.description}</p>
                  <div className="flex items-center justify-between">
                    <span className="text-white/40 text-xs truncate mr-2">📍 {e.venue}</span>
                    {e.googleFormLink && (
                      <a href={toAbsoluteUrl(e.googleFormLink)} target="_blank" rel="noreferrer"
                        className="shrink-0 px-3 py-1.5 rounded-lg bg-linear-to-r from-purple-500 to-blue-500 hover:from-purple-600 hover:to-blue-600 transition text-xs font-semibold">
                        Register →
                      </a>
                    )}
                  </div>
